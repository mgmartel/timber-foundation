<?php
/**
 * Instead of using TimberMenu, we're using this walker, mainly because walkers
 * allow for infinite nesting and fit in better with the rest of the WP ecosystem.
 *
 * The walker below comes from Reverie, and has been exapted from many walkers
 * before it. See the comment above the class. Standing on the shoulders of giants!
 */
 ...
 * From Reverie Foundation https://github.com/milohuang/reverie/
 * In turn, from required+ Foundation http://themes.required.ch
 *
 ...
 *
 */
class TimberFoundation_Walker extends Walker_Nav_Menu {
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        if ( $is_button = in_array( 'call-to-action', $classes ) ) {
            // Replace the parent class with 'has-form'
            $classes[array_search( 'call-to-action', $classes )] = 'has-form';
        }
		...
		if ( $depth > 0 ) {
			...
			$output .= $indent . ( $this->nav_bar['in_top_bar'] && $this->nav_bar['add_dividers'] ? '<li class="divider"></li>' : '' ) . '<' . $this->nav_bar['item_type'] . ' id="menu-item-'. $item->ID . '"' . $value . $class_names .'>';
		}
        ...
        $attributes .= $is_button                   ? ' class="button"' : '';
    ...
    // UTIL
    static function render_menu_fallback() {
